...
const TABS = [
  { id: "execution", label: "Esecuzione", icon: PlayCircle },
  { id: "logs", label: "Log", icon: TerminalSquare },
];
...
function Ingest() {
  ...
  const currentBank = sessionStorage.getItem("selectedBank") || "default";
  const generalParamsKey = `generalParams_${currentBank}`;
  ...
  // 🔹 Persistenza generalParams (week, year) specifica per banca
  const [generalParams, setGeneralParams] = useState(() => {
    const saved = localStorage.getItem(generalParamsKey);
    return saved
      ? JSON.parse(saved)
      : { selectedWeek: "1", selectedYear: currentYear.toString() };
  });
  ...
  useEffect(() => {
    ...
    logsData.forEach((log) => {
      if (!headersMap[log.logKey]) {
        headersMap[log.logKey] = {
          logKey: log.logKey,
          executed_by: log.executed_by || "N/A",
          status: log.status,
  ...
  useEffect(() => {
    if (!metadataFilePath) {
      toast.error(
        "Nessun file Excel selezionato. Torna alla home e seleziona un file."
      );
      navigate("/home");
      return;
    }
  ...
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === "executionTabFilters" && e.newValue)
        setExecutionFilters(JSON.parse(e.newValue));
      if (e.key === "executionTabSortConfig" && e.newValue)
        setExecutionSortConfig(JSON.parse(e.newValue));
      if (e.key === generalParamsKey && e.newValue)
        setGeneralParams(JSON.parse(e.newValue));
  ...
  // 🔹 Reset selezioni quando cambiano i filtri (solo se "seleziona tutti" era attivo)
  const prevFiltersRef = useRef({ flowSearchTerm, packageFilter, statusFilter });
  const prevDisplayedFlowsRef = useRef(displayedFlows);

  useEffect(() => {
    const prev = prevFiltersRef.current;
    const prevDisplayed = prevDisplayedFlowsRef.current;

    const filtersChanged =
      prev.flowSearchTerm !== flowSearchTerm ||
      prev.packageFilter !== packageFilter ||
      prev.statusFilter !== statusFilter;

    // Verifica se tutti i flussi visualizzati PRIMA del cambio filtro erano selezionati
    const allPreviousSelected = prevDisplayed.length > 0 &&
      prevDisplayed.every(flow => selectedFlows.has(flow.id));

    // Resetta solo se i filtri sono cambiati E "seleziona tutti" era attivo
    if (filtersChanged && allPreviousSelected) {
      setSelectedFlows(new Set());
    }

    prevFiltersRef.current = { flowSearchTerm, packageFilter, statusFilter };
    prevDisplayedFlowsRef.current = displayedFlows;
  }, [flowSearchTerm, packageFilter, statusFilter, displayedFlows]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        return (
          <ExecutionTabContent
            ...
            handleGeneralParamChange={(key, value) => {
              ...
              localStorage.setItem(generalParamsKey, JSON.stringify(updated));
            }}
            sortConfig={executionSortConfig}
            requestSort={(key) => {
              let direction = "asc";
              if (
                executionSortConfig.key === key &&
                executionSortConfig.direction === "asc"
              )
                direction = "desc";
              const newSort = { key, direction };
              setExecutionSortConfig(newSort);
              localStorage.setItem(
                "executionTabSortConfig",
